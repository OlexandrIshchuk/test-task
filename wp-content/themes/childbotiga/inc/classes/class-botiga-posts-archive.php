		...
		*/
		...
		public function post_excerpt() {
			...
			if ( in_array( get_post_format(), array( 'aside', 'quote', 'link', 'image', 'video', 'status' ) ) ) {
				...
				echo '<div class="price-and-quantity-row">';
					echo '<div class="price"><h5>';
						the_field('currency');
						the_field('price');
					echo '</h5></div>';
					echo '<div>';
				...
				if ( $read_more ) {
					echo '<button class="quick-review-button"><a title="' . esc_attr( strip_tags( get_the_title() ) ) . '" href="' . esc_url( get_permalink() ) . '">' . esc_html__( 'סקירה מהירה', 'botiga' ) . '</a></button>';
				}
				echo '</div>';
